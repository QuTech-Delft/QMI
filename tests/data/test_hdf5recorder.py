...
class TestRecorderH5py(unittest.TestCase):
    ...
    def test_record(self):
        ...
        for (q1, q2) in [(1234, 2345), (2345, 4444), (3000, 5000), (4010, 8000), (5000, 11000), (5000, 15000)]:
            rec.record(self._dims[0], test_data1[p1:q1])
            rec.record(self._dims[1], test_data2[p2:q2])
            p1 = q1
            p2 = q2
            time.sleep(0.8)
        ...
        try:
            self.assertEqual(set(hdf.keys()), {self._dims[0], self._dims[1]})
            self.assertEqual(hdf[self._dims[0]].shape, test_data1.shape)
            self.assertEqual(hdf[self._dims[0]].dtype, test_data1.dtype)
            self.assertTrue(np.all(hdf[self._dims[0]][:] == test_data1))
            self.assertEqual(hdf[self._dims[1]].shape, test_data2.shape)
            self.assertEqual(hdf[self._dims[1]].dtype, test_data2.dtype)
            self.assertTrue(np.all(hdf[self._dims[1]][:] == test_data2))
    ...
    def test_attributes(self):
        ...
        # Write to dataset.
        rec.record(self._dims[0], test_data1)
        time.sleep(1.2)

        # Add attribute to existing dataset.
        rec.set_attribute(self._dims[0], "one", 1)
        rec.set_attribute(self._dims[0], "more", "MOAR")

        # Add attribute to non-existing datasets.
        rec.set_attribute(self._dims[1], "two", 2.0)
        rec.set_attribute("d3", "three", 3)
        time.sleep(1.2)

        # Overwrite existing attribute and add new attribute
        rec.set_attribute(self._dims[0], "more", "less")
        rec.set_attribute(self._dims[0], "neg", -1000)
        time.sleep(1.2)

        # Create dataset that already has pending attribute.
        rec.record(self._dims[1], test_data2)
        rec.close()
        ...
        try:
            ...
            self.assertEqual(hdf[self._dims[0]].dtype, test_data1.dtype)
            self.assertTrue(np.all(hdf[self._dims[0]][:] == test_data1))
            self.assertEqual(dict(hdf[self._dims[0]].attrs), {"one": 1, "more": "less", "neg": -1000})
            self.assertEqual(hdf[self._dims[1]].shape, test_data2.shape)
            self.assertEqual(hdf[self._dims[1]].dtype, test_data2.dtype)
            self.assertTrue(np.all(hdf[self._dims[1]][:] == test_data2))
            self.assertEqual(dict(hdf[self._dims[1]].attrs), {"two": 2.0})

        finally:
            hdf.close()


class TestRecorderH5NetCdf(unittest.TestCase):
    ...
    def test_record(self):
        ...
        file_name = os.path.join(self._dir.name, "test_record.h5")
        rec = HDF5Recorder(file_name, write_interval=1.0, backend="h5netcdf")
        ...
        # Check HDF5 file contents.
        hdf = h5netcdf.File(file_name, "r", decode_vlen_strings=False)
        try:
            self.assertEqual(set(hdf.keys()), {self._dims[0], self._dims[1]})
            self.assertEqual(hdf[self._dims[0]].shape, test_data1.shape)
            self.assertEqual(hdf[self._dims[0]].dtype, test_data1.dtype)
