...
class AptMessageId(Enum):
    """Possible message IDs for messages to the Thorlabs MPC320."""
    HW_REQ_INFO = 0x0005
    HW_GET_INFO = 0x0006
    MOD_IDENTIFY = 0X0223
...
class AptProtocol:
    ...
    def __init__(self,
        ...
        Parameters:
            ...
            apt_device_address: The address of the APT device. By default it is 0x50 which is a generic USB hardware unit.
            host_address:       The address of the host that sends and receives messages. By default it is 0x01 which is
                                a host controller such as a PC.
    ...
    def write_param_command(self, message_id: int, param1: Optional[int] = None, param2: Optional[int] = None) -> None:
        ...
        """
        # Make the command.
        msg = AptMessageHeaderWithParams(message_id, param1 or 0x00, param2 or 0x00, self._apt_device_address, self._host_address)
    ...
    def ask(self, data_type: APT_DATA_PACKET_TYPE, timeout: Optional[float] = None) -> APT_DATA_PACKET_TYPE:
        ...
        if data_type.MESSAGE_ID != header.message_id:
                raise QMI_InstrumentException(f"Expected message with ID {data_type.MESSAGE_ID}, but received {header.message_id}")
