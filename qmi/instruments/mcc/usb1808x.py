...
class _Mcc_Usb1808xWindows:
    ...
    def open(self) -> None:
        ...
        try:
            ...
            self._device_info = device_info.DaqDeviceInfo(self.board_num)
    ...
    def set_dio_direction(self, channel: int, output: bool) -> None:
        ...
        port = self.device_info.get_dio_info().port_info[channel]
        ul.d_config_port(self._board_id, port.type, d)
    ...
    def get_dio_input_bit(self, channel: int) -> bool:
        ...
        """
        port = self.device_info.get_dio_info().port_info[channel]
        bit_num = port.first_bit + min(port.num_bits, 8)  # TODO: Or simply port.first_bit?
        ...
        # }
        return ul.d_bit_in(self._board_id, port.type, bit_num)
    ...
    def set_dio_output_bit(self, channel: int, value: bool) -> None:
        ...
        port = self.device_info.get_dio_info().port_info[channel]
        bit_num = port.first_bit + min(port.num_bits, 8)  # TODO: Or simply port.first_bit?
        return ul.d_bit_out(self.board_id, port.type, bit_num, int(value))
